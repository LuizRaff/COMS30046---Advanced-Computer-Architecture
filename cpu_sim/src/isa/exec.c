...
word_t do_alu(opcode_t op, word_t a, word_t b, word_t imm) {
  switch (op) {
    case OP_ADD: return (word_t)(a + b);
    case OP_ADDI: return (word_t)(a + imm);
    case OP_SUB: return (word_t)(a - b);
    case OP_SUBI: return (word_t)(a - imm);
    case OP_MUL: return (word_t)(a * b);
    case OP_MULI: return (word_t)(a * imm);
    case OP_AND: return (word_t)(a & b);
    case OP_OR: return (word_t)(a | b);
    case OP_XOR: return (word_t)(a ^ b);
    case OP_NOT: return (word_t)(~a);
    case OP_CMP:
      if (a < b) return (word_t)(-1);
      else if (a == b) return (word_t)(0);
      else return (word_t)(1);
    case OP_SHR: {
...
void do_vadd(word_t *dst, const word_t *a, const word_t *b) {
  for (int i = 0; i < VLEN; i++) dst[i] = a[i] + b[i];
}
