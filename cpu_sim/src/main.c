...
static size_t bm_independent_alu(instr_t *p) {
  ...
  for (int i = 1; i <= 20; i++)
    p[n++] =
        (instr_t){.op = OP_ADDI, .rd = i, .rs1 = 0, .imm = i, .has_imm = true};
...
static size_t bm_dependent_alu(instr_t *p) {
  ...
  for (int i = 0; i < 30; i++)
    p[n++] =
        (instr_t){.op = OP_ADDI, .rd = 1, .rs1 = 1, .imm = 1, .has_imm = true};
  p[n++] = (instr_t){.op = OP_HALT};
  return n;
}
...
static size_t bm_branch_loop(instr_t *p) {
  int n = 0;
  p[n++] =
      (instr_t){.op = OP_LDC, .rd = 1, .imm = 0, .has_imm = true}; /* 0: i=0 */
  p[n++] = (instr_t){
      .op = OP_LDC, .rd = 2, .imm = 20, .has_imm = true}; /* 1: limit */
  /* loop: */
  p[n++] = (instr_t){
      .op = OP_ADDI, .rd = 1, .rs1 = 1, .imm = 1, .has_imm = true}; /* 2: i++ */
  p[n++] = (instr_t){.op = OP_BLTH,
                     .rs1 = 1,
                     .rs2 = 2,
                     .imm = 2,
                     .has_imm = true}; /* 3: if i<limit goto 2 */
  p[n++] = (instr_t){.op = OP_HALT};
  return n;
}
...
static size_t bm_branch_pattern(instr_t *p) {
  ...
  p[n++] =
      (instr_t){.op = OP_LDC, .rd = 1, .imm = 0, .has_imm = true}; /* 0: i=0 */
  p[n++] = (instr_t){
      .op = OP_LDC, .rd = 2, .imm = 30, .has_imm = true}; /* 1: limit */
  p[n++] = (instr_t){
      .op = OP_LDC, .rd = 3, .imm = 2, .has_imm = true}; /* 2: const 2 */
  /* loop: */
  p[n++] = (instr_t){
      .op = OP_AND, .rd = 4, .rs1 = 1, .rs2 = 3, .has_imm = false}; /* 3: i&2 */
  p[n++] = (instr_t){.op = OP_BLTH,
                     .rs1 = 0,
                     .rs2 = 4,
                     .imm = 6,
                     .has_imm = true}; /* 4: if 0<(i&2) goto 6 */
  p[n++] = (instr_t){.op = OP_ADDI,
                     .rd = 5,
                     .rs1 = 5,
                     .imm = 1,
                     .has_imm = true}; /* 5: nt path */
  /* taken target: */
  p[n++] = (instr_t){.op = OP_ADDI,
                     .rd = 6,
                     .rs1 = 6,
                     .imm = 1,
                     .has_imm = true}; /* 6: t path */
  p[n++] = (instr_t){
      .op = OP_ADDI, .rd = 1, .rs1 = 1, .imm = 1, .has_imm = true}; /* 7: i++ */
  p[n++] = (instr_t){.op = OP_BLTH,
                     .rs1 = 1,
                     .rs2 = 2,
                     .imm = 3,
                     .has_imm = true}; /* 8: loop back to 3 */
  p[n++] = (instr_t){.op = OP_HALT};
  return n;
}
...
static size_t bm_scalar_vector_add(instr_t *p) {
  ...
  /* loop: */
  p[n++] = (instr_t){
      .op = OP_LD, .rd = 7, .rs1 = 2, .imm = 0, .has_imm = true}; /* 5 */
  p[n++] = (instr_t){
      .op = OP_LD, .rd = 8, .rs1 = 3, .imm = 0, .has_imm = true}; /* 6 */
  p[n++] = (instr_t){
      .op = OP_ADD, .rd = 9, .rs1 = 7, .rs2 = 8, .has_imm = false}; /* 7 */
  p[n++] = (instr_t){
      .op = OP_ST, .rs1 = 1, .rs2 = 9, .imm = 0, .has_imm = true}; /* 8 */
  p[n++] =
      (instr_t){.op = OP_ADDI, .rd = 1, .rs1 = 1, .imm = 4, .has_imm = true};
  p[n++] =
      (instr_t){.op = OP_ADDI, .rd = 2, .rs1 = 2, .imm = 4, .has_imm = true};
  p[n++] =
      (instr_t){.op = OP_ADDI, .rd = 3, .rs1 = 3, .imm = 4, .has_imm = true};
  p[n++] =
      (instr_t){.op = OP_ADDI, .rd = 4, .rs1 = 4, .imm = 1, .has_imm = true};
  p[n++] =
      (instr_t){.op = OP_BLTH, .rs1 = 4, .rs2 = 5, .imm = 5, .has_imm = true};
  p[n++] = (instr_t){.op = OP_HALT};
  return n;
}
...
static size_t bm_vectorized_vector_add(instr_t *p) {
  ...
  p[n++] = (instr_t){.op = OP_LDC, .rd = 4, .imm = 0, .has_imm = true};
  p[n++] =
      (instr_t){.op = OP_LDC, .rd = 5, .imm = ARRAY_N / VLEN, .has_imm = true};
  /* loop: */
  p[n++] = (instr_t){.op = OP_VLD,
                     .rd = 0,
                     .rs1 = 2,
                     .imm = 0,
                     .has_imm = true}; /* v0 = mem[B] */
  p[n++] = (instr_t){.op = OP_VLD,
                     .rd = 1,
                     .rs1 = 3,
                     .imm = 0,
                     .has_imm = true}; /* v1 = mem[C] */
  p[n++] = (instr_t){.op = OP_VADD,
                     .rd = 2,
                     .rs1 = 0,
                     .rs2 = 1,
                     .has_imm = false}; /* v2 = v0+v1 */
  p[n++] =
      (instr_t){.op = OP_VST,
                .rs1 = 1,
                .rs2 = 2,
                .imm = 0,
                .has_imm = true}; /* mem[A] = v2 -- note rs1=r1 (A_BASE ptr) */
  /* VST uses rs1 as the scalar base register and rs2 as the vector source
   * register. */
  p[n++] = (instr_t){
...
int main(int argc, char **argv) {
  ...
  cpu_config_t cfg = {.issue_width = issue_width,
                      .num_alus = num_alus,
                      .num_lsus = 1,
                      .num_brus = 1,
