...
static void mem_fault(const char *msg, uint32_t addr) {
...
word_t memory_load_w(const memory_t *m, uint32_t addr_bytes) {
    if (addr_bytes & 3u) mem_fault("unaligned word load", addr_bytes);
    uint32_t idx = addr_bytes >> 2;
    if (idx >= m->nwords) mem_fault("out of bounds load", addr_bytes);
    return m->words[idx];
}

void memory_store_w(memory_t *m, uint32_t addr_bytes, word_t value) {
    if (addr_bytes & 3u) mem_fault("unaligned word store", addr_bytes);
    uint32_t idx = addr_bytes >> 2;
    if (idx >= m->nwords) mem_fault("out of bounds store", addr_bytes);
    m->words[idx] = value;
}
