...
void inst_to_str(char *buf, size_t n, const instr_t *inst) {
  ...
  case OP_BLTH:
    ...
    break;
  case OP_VLD:
    snprintf(buf, n, "%s v%d,[r%d%+d]", op, inst->rd, inst->rs1,
             (int)inst->imm);
    break;
  case OP_VST:
    snprintf(buf, n, "%s v%d->[r%d%+d]", op, inst->rs2, inst->rs1,
             (int)inst->imm);
    break;
  case OP_VADD:
  case OP_VMUL:
    snprintf(buf, n, "%s v%d,v%d,v%d", op, inst->rd, inst->rs1, inst->rs2);
    break;
  case OP_NOP:
  case OP_HALT:
    snprintf(buf, n, "%s", op);
...
static int bp_predict(cpu_t *cpu, uint32_t pc, const instr_t *inst) {
  ...
  case BP_ALWAYS_TAKEN:
    return 1;
  case BP_TWO_BIT:
    return cpu->bp_table[pc % BP_TABLE] >= 2;
...
static void bp_update(cpu_t *cpu, uint32_t pc, int taken) {
  ...
  case BP_TWO_LEVEL_LOCAL: {
    ...
    if (taken && *c < 3)
      ...
      (*c)--;
    cpu->local_hist[idx] = ((cpu->local_hist[idx] << 1) | (taken ? 1 : 0)) &
                           ((1 << LOCAL_HIST_BITS) - 1);
    break;
  }
  }
...
int tick(cpu_t *cpu, const instr_t *program, size_t program_len) {
  ...
  for (int c = 0; c < cpu->cfg.issue_width && !flush; c++) {
    ...
    if (h->type == ROB_TYPE_REG_WRITE) {
      if (h->dest_reg > 0)
        reg_set(&cpu->rf, h->dest_reg, h->result);
      ...
      if (h->dest_vreg >= 0 && h->dest_vreg < NUM_VREG)
        for (int v = 0; v < VLEN; v++)
          cpu->vreg[h->dest_vreg].e[v] = h->vec_result[v];
    } else if (h->type == ROB_TYPE_VEC_STORE) {
      uint32_t addr = addr_of(h->result, h->inst.imm);
      for (int v = 0; v < VLEN; v++)
        mem_store(&cpu->mem, addr + v * 4, h->vec_result[v]);
  ...
  for (int i = 0; i < cpu->num_eus; i++) {
    ...
    for (int j = 0; j < RS_SIZE; j++) {
      ...
      if (cpu->rs[j].qj == dr) {
        cpu->rs[j].qj = -1;
        cpu->rs[j].vj = cpu->eus[i].result;
      }
      if (cpu->rs[j].qk == dr) {
        cpu->rs[j].qk = -1;
        cpu->rs[j].vk = cpu->eus[i].result;
      }
    }
    cpu->eus[i].busy = 0;
  }
  ...
  for (int i = 0; i < cpu->num_eus; i++) {
    if (cpu->eus[i].busy)
      continue;
    for (int j = 0; j < RS_SIZE; j++) {
      if (!cpu->rs[j].busy || cpu->rs[j].req_eu != cpu->eus[i].type)
        continue;
      ...
      if (cpu->eus[i].type == EU_TYPE_ALU) {
        ...
        int taken = 0;
        uint32_t tpc = cpu->rs[j].pc + 1;
        if (inst->op == OP_B) {
          taken = 1;
          tpc = (uint32_t)imm;
        } else if (inst->op == OP_J) {
          taken = 1;
          tpc = cpu->rs[j].pc + (uint32_t)imm;
        } else if (inst->op == OP_BLTH) {
          if (vj < vk) {
            taken = 1;
            tpc = (uint32_t)imm;
          }
        ...
        if (rb->branch_taken != taken || (taken && rb->correct_pc != tpc)) {
          cpu->mispredictions++;
          cpu->eus[i].flag_extra = 1;
          cpu->eus[i].result_extra = tpc;
        } else {
        ...
        if (inst->op == OP_VLD) {
          ...
          if (vs2 >= 0 && vs2 < NUM_VREG)
            memcpy(b, cpu->vreg[vs2].e, sizeof(b));
          if (inst->op == OP_VADD)
            do_vadd(cpu->eus[i].vec_result, a, b);
          else
            do_vmul(cpu->eus[i].vec_result, a, b);
        }
        (void)rb;
      }
      cpu->rs[j].busy = 0;
