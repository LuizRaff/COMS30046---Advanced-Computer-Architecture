...
static void format_inst(char *buf, size_t n, const instr_t *inst) {
    // Small human-readable formatter for debugging.
    const char *op = opcode_name(inst->op);
    switch (inst->op) {
        case OP_LD:
            snprintf(buf, n, "%s r%d, [r%d %+d]", op, inst->rd, inst->rs1, (int)inst->imm);
            break;
        case OP_ST:
            snprintf(buf, n, "%s r%d -> [r%d %+d]", op, inst->rs2, inst->rs1, (int)inst->imm);
            break;
        case OP_LDC:
        ...
        case OP_J:
            ...
            break;
        case OP_BLTH:
            snprintf(buf, n, "%s r%d, r%d, %d", op, inst->rs1, inst->rs2, (int)inst->imm);
            break;
...
static void cpu_dump_pipeline_fp(const cpu_t *cpu, FILE *out) {
    ...
    if (cpu->if_id.valid) format_inst(if_buf, sizeof(if_buf), &cpu->if_id.inst);
    else snprintf(if_buf, sizeof(if_buf), "<empty>");

    if (cpu->id_ex.valid) format_inst(id_buf, sizeof(id_buf), &cpu->id_ex.inst);
    else snprintf(id_buf, sizeof(id_buf), "<empty>");

    fprintf(out,
            "[cycle=%" PRIu64 "] pc=%u halted=%d\n"
            "  IF/ID: valid=%d pc=%u  %s\n"
            "  ID/EX: valid=%d pc=%u  %s\n",
            cpu->cycles, cpu->pc, cpu->halted,
            cpu->if_id.valid, cpu->if_id.pc, if_buf,
            cpu->id_ex.valid, cpu->id_ex.pc, id_buf);
}

void cpu_dump_pipeline(const cpu_t *cpu) {
    cpu_dump_pipeline_fp(cpu, stderr);
}
...
void cpu_print_stats(const cpu_t *cpu) {
    double ipc = (cpu->cycles == 0) ? 0.0 : ((double)cpu->instrs / (double)cpu->cycles);
    printf("instructions=%" PRIu64 " cycles=%" PRIu64 " IPC=%.3f\n",
           cpu->instrs, cpu->cycles, ipc);
}
...
static instr_t nop_instr(void) {
    return (instr_t){ .op = OP_NOP, .rd = 0, .rs1 = 0, .rs2 = 0, .imm = 0, .has_imm = false };
}

int cpu_step(cpu_t *cpu, const instr_t *program, size_t program_len) {
    // Finished when: no more fetch AND pipeline drained.
    if ((cpu->halted || cpu->pc >= program_len) && !cpu->if_id.valid && !cpu->id_ex.valid) {
        return 1;
    }
    ...
    if (cpu->id_ex.valid) {
        ...
        switch (inst->op) {
            ...
            case OP_HALT:
                // Stop fetching and squash any younger in-flight instructions.
                cpu->halted = 1;
                redirect = 1;
    ...
    // -----------------------------
    pipe_reg_t if_id_next = (pipe_reg_t){ .inst = nop_instr(), .pc = 0, .valid = 0 };
    ...
    // Apply redirect/flush (branch taken or halt) at end of cycle.
    if (redirect) {
...
int cpu_run_dump(cpu_t *cpu,
    ...
    if (dump_filename == NULL || dump_filename[0] == '\0') {
        fprintf(stderr, "cpu_run_dump: dump_filename is required when dump_enable=true\n");
        return -1;
    }
    ...
    // Optional header + initial state (cycle 0).
